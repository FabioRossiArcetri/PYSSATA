...
class ModulatedPyramid(BaseProcessingObj):
    def __init__(self, wavelength_in_nm, fov_res, fp_masking, fft_res, tilt_scale, fft_sampling, 
        ...
        self._psf_tot = BaseValue(xp.zeros((int(cpuArray(fft_totsize)), int(cpuArray(fft_totsize)))))
        self._psf_bfm = BaseValue(xp.zeros((int(cpuArray(fft_totsize)), int(cpuArray(fft_totsize)))))
