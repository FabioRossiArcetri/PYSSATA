...
class IIRControl(TimeControl, BaseProcessingObj):
    ...
    def __init__(self, iirfilter, delay=0):
        ...
        self.inputs['in_delta_comm'] = InputValue(object=self._delta_comm, type=xp.ndarray)
    ...
    def set_modal_start_time(self, modal_start_time):
        modal_start_time_ = xp.array(modal_start_time)
        ...
        if self._delta_comm.generation_time == t:
            ...
            if self._start_time > 0 and self._start_time > t:
                ...
                if self._do_gmt_init_mod_manager:
                    time_idx = self._time_gmt_imm if self._time_gmt_imm is not None else xp.zeros(0)
                    gain_idx = self._gain_gmt_imm if self._gain_gmt_imm is not None else xp.zeros(0)
                ...
                if len(delta_comm) < self._iirfilter.nfilter:
                    ...
                    delta_comm = xp.zeros(self._iirfilter.nfilter)
                ...
                if self._doExtraPol:
                    ...
                    deltaCommHist = self._deltaCommHistEx if self._deltaCommHistEx is not None else xp.zeros((0, 0))
                    commHist = self._commHistEx if self._commHistEx is not None else xp.zeros((0, 0))
                    deltaCommFutureHist = self._deltaCommFutureHistEx if self._deltaCommFutureHistEx is not None else xp.zeros((0, 0))
                    if LPF:
                        ostMat = self._ostMatEx if self._ostMatEx is not None else xp.zeros((0, 0))
                        istMat = self._istMatEx if self._istMatEx is not None else xp.zeros((0, 0))
