...
class SlopecDisplay(BaseProcessingObj):
    ...
    def trigger(self, t):
        slopec = self.inputs['slopec'].get(self._device_idx)
        s = slopec.out_slopes
...
#                    slope_side = nx * 2
                ...
                _, _, _, self.fig, self.ax = pupil_display(cpuArray(slopec.inputs['in_pixels'].get(self._device_idx).pixels), cpuArray(sx), cpuArray(sy), 
                                                           cpuArray(map_data), slopec.inputs['in_pixels'].get(self._device_idx).pixels.shape[0], title=title, TARGET=TARGET,
                                                           do_image_show=True)
