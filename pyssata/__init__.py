...
# and still want to use the CPU (idx==-1) as default_device
...
def init(device_idx=-1, precision=0):
    ...
    global default_device_idx
    global default_device
    
    default_device_idx = device_idx
    ...
    if default_device_idx>=0:
        ...
        default_device = cp.cuda.Device(default_device_idx)
        default_device.use()
        print('Default device is GPU number ', default_device_idx)
        # print('Using device: ', cp.cuda.runtime.getDeviceProperties(default_device)['name'])
        # attributes = default_device.attributes
        # properties = cp.cuda.runtime.getDeviceProperties(default_device)
