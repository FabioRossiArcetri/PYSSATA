...
def cv_coord(from_rect=None, from_polar=None, from_cylin=None, from_sphere=None,
    ...
    if from_rect is not None:
        from_rect = xp.array(from_rect, dtype=standard_dtype)
        if double:
        ...
        if to_polar:
            ...
            return xp.array([ang, rad], dtype=standard_dtype)
        
        if to_cylin:
            rad = xp.sqrt(from_rect[0]**2 + from_rect[1]**2)
            ang = xp.arctan2(from_rect[1], from_rect[0]) * ang_out
            z = from_rect[2] if from_rect.shape[0] > 2 else zero
            return xp.array([ang, rad, z], dtype=standard_dtype)
        ...
        if to_sphere:
            ...
            return xp.array([ang1, ang2, rad], dtype=standard_dtype)
        
        return from_rect
    
    elif from_polar is not None:
        from_polar = xp.array(from_polar, dtype=standard_dtype)
        if double:
        ...
        if to_rect:
            ...
            return xp.array([x, y], dtype=standard_dtype)
        
        if to_cylin:
            z = zero
            return xp.array([from_polar[0], from_polar[1], z], dtype=standard_dtype)
        
        if to_sphere:
            return xp.array([from_polar[0], zero, from_polar[1]], dtype=standard_dtype)
        
        return from_polar
    
    elif from_cylin is not None:
        from_cylin = xp.array(from_cylin, dtype=standard_dtype)
        if double:
        ...
        if to_rect:
            ...
            z = from_cylin[2]
            return xp.array([x, y, z], dtype=standard_dtype)
        
        if to_polar:
            return xp.array([from_cylin[0], from_cylin[1]], dtype=standard_dtype)
        ...
        from_sphere = xp.array(from_sphere, dtype=standard_dtype)
        if double:
        ...
        if to_polar:
            ...
            return xp.array([from_sphere[0], rad], dtype=standard_dtype)
        
        if to_cylin:
            rad = from_sphere[2] * xp.cos(from_sphere[1] * ang_in)
            z = from_sphere[2] * xp.sin(from_sphere[1] * ang_in)
            return xp.array([from_sphere[0], rad, z], dtype=standard_dtype)
